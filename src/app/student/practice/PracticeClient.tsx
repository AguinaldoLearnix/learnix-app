...
export function PracticeClient({ context }: Props) {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden">
            <ChatWindow mode={mode} context={context} />
          </div>
        </div>
        ...
        <div className="w-[260px] shrink-0 border-l flex flex-col overflow-y-auto"
          ...
          <div className="p-4 border-b space-y-3" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
            <p className="text-[10px] font-medium uppercase tracking-[0.08em]" style={{ color: '#555' }}>Esta semana</p>
            {context.theme ? (
              <div className="space-y-1">
                <p className="text-[13px] font-medium text-white">{context.theme}</p>
                <p className="text-[11px]" style={{ color: '#666' }}>{context.grammar_focus}</p>
                <p className="text-[11px] mt-2" style={{ color: '#555' }}>
                  Nível: <span className="text-white">{context.current_level}</span> → <span className="text-white">{context.target_level}</span>
                </p>
              </div>
            ) : (
              <p className="text-[12px]" style={{ color: '#555' }}>Nenhuma unidade ativa</p>
            )}
          </div>
          ...
          <div className="p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[10px] font-medium uppercase tracking-[0.08em]" style={{ color: '#555' }}>Modos</p>
            </div>
